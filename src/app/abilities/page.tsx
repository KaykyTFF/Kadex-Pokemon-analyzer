...
import { Search, Filter, ChevronRight, Zap, ShieldCheck, Users } from "lucide-react";
import { MOCK_ABILITIES, Ability } from "@/data/mock/abilities";
import Link from "next/link";

const CATEGORIES = [
  "All", "Offensive", "Defensive", "Utility", "Weather", "Terrain", "Speed Control"
];
...
export default function AbilitiesPage() {
  ...
  const filteredAbilities = MOCK_ABILITIES.filter(ability => {
    const matchesSearch = ability.name.toLowerCase().includes(search.toLowerCase()) || 
                          ability.description.toLowerCase().includes(search.toLowerCase()) ||
                          ability.competitiveEffect.toLowerCase().includes(search.toLowerCase());
    const matchesCategory = activeCategory === "All" || ability.category === activeCategory;
    ...
    <div className="kadex-page-container rc-animate-fade pb-32">
      ...
      <div className="space-y-2 border-b border-slate-100 pb-8 mb-8 md:mb-12">
        ...
        <p className="text-sm text-slate-500 font-medium max-w-xl">
          Explore Pokémon abilities and their impact on competitive play, featuring tactical effects and common users.
        </p>
      </div>
      ...
      {filteredAbilities.length > 0 ? (
        ...
        <div className="bg-white border border-dashed border-slate-200 rounded-[40px] py-32 text-center space-y-4">
           ...
           <p className="text-xs font-black uppercase tracking-[0.2em] text-slate-400 italic">No abilities found matching your filters</p>
        </div>
      )}
    </div>
  );
